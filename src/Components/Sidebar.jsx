...
const NavItem = ({ icon, label, active, onClick }) => (
  <div 
    ...
    className={`group flex items-center justify-between p-4 rounded-2xl cursor-pointer transition-all duration-300 border
      ${active 
        ? 'bg-blue-600/10 border-blue-600/20 text-blue-600 dark:text-blue-400 shadow-[0_0_20px_rgba(37,99,235,0.1)]' 
  ...
  >
    <div className="flex items-center gap-4">
      <span className={`transition-transform duration-300 group-hover:scale-110 ${active ? 'text-blue-600 dark:text-blue-400' : ''}`}>
        {icon}
      </span>
    ...
    {!active && (
      <ChevronRight size={14} className="opacity-0 group-hover:opacity-100 -translate-x-2 group-hover:translate-x-0 transition-all text-slate-300 dark:text-slate-600" />
    )}
  </div>
);
