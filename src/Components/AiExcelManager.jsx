import React, { useState, useRef, useEffect, useMemo } from 'react';
import * as XLSX from 'xlsx';
import { Send, BrainCircuit, FileUp, Loader2, TrendingUp, Download } from 'lucide-react';
import Groq from "groq-sdk";

// 🔑 API Client Setup
const apiKey = import.meta.env.VITE_GROQ_API_KEY;
const groq = new Groq({ apiKey: apiKey, dangerouslyAllowBrowser: true });

const AiExcelManager = () => {
  const [messages, setMessages] = useState([{ 
    role: 'ai', 
    text: "Beer AI v9.0 Live! Bhai ab sab fix hai. Aik entry kar ke check karo!" 
  }]);
  const [excelData, setExcelData] = useState([]);
  const [input, setInput] = useState("");
  const [isTyping, setIsTyping] = useState(false);
  const chatEndRef = useRef(null);

  // Auto Scroll
  useEffect(() => chatEndRef.current?.scrollIntoView({ behavior: "smooth" }), [messages]);

  // 📈 Math Engine: Auto Balance Calculation
  const processedData = useMemo(() => {
    let currentBalance = 0;
    return excelData.map((row) => {
      const credit = parseFloat(row.Credit) || 0;
      const debit = parseFloat(row.Debit) || 0;
      currentBalance = currentBalance + credit - debit;
      return { ...row, Balance: currentBalance };
    });
  }, [excelData]);

  // 📂 Excel File Loader
  const handleFileUpload = (e) => {
    const file = e.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = (evt) => {
      const wb = XLSX.read(evt.target.result, { type: 'binary' });
      const wsname = wb.SheetNames[0];
      const data = XLSX.utils.sheet_to_json(wb.Sheets[wsname]);
      setExcelData(data);
      setMessages(prev => [...prev, { role: 'ai', text: `Bhai, ${data.length} records load ho gaye!` }]);
    };
    reader.readAsBinaryString(file);
  };

  // 💬 Fixed Chat Logic (Surgical Precision)
  const handleSend = async () => {
    if (!input.trim()) return;
    if (!apiKey) {
      setMessages(prev => [...prev, { role: 'ai', text: "Bhai, API Key nazar nahi aa rahi!" }]);
      return;
    }

    const userQuery = input;
    setInput("");
    setMessages(prev => [...prev, { role: 'user', text: userQuery }]);
    setIsTyping(true);

    try {
      const completion = await groq.chat.completions.create({
        messages: [
          { 
            role: "system", 
            content: `You are 'Beer AI', a professional accountant. 
            Rules:
            1. Always respond with [MESSAGE]: followed by confirmation.
            2. Always include [DATA]: followed by the COMPLETELY UPDATED JSON array.
            3. Columns: "Date", "Customer Name", "Description", "Debit", "Credit".
            4. If user says 'entry karo', add it to the existing list.` 
          },
          { 
            role: "user", 
            content: `Current Data: ${JSON.stringify(excelData)}\nCommand: ${userQuery}` 
          }
        ],
        model: "llama-3.3-70b-versatile",
        temperature: 0.1, // High accuracy
      });

      const responseText = completion.choices[0]?.message?.content || "";
      
      // Parsing Logic
      const dataParts = responseText.split("[DATA]:");
      const messageParts = dataParts[0].split("[MESSAGE]:");
      
      const aiMsg = messageParts[1] ? messageParts[1].trim() : "Entry updated, bhai!";
      const aiDataRaw = dataParts[1] ? dataParts[1].trim() : null;

      if (aiDataRaw) {
        try {
          const cleanedJson = aiDataRaw.match(/\[[\s\S]*\]/)[0]; // Extract only the array
          const parsedData = JSON.parse(cleanedJson);
          setExcelData(parsedData);
          setMessages(prev => [...prev, { role: 'ai', text: aiMsg }]);
        } catch (jsonErr) {
          console.error("JSON Parse Error", jsonErr);
          setMessages(prev => [...prev, { role: 'ai', text: "Bhai data toh aaya par format thora messy tha. Dobara try karein." }]);
        }
      } else {
        setMessages(prev => [...prev, { role: 'ai', text: aiMsg }]);
      }
    } catch (err) {
      console.error("API Error", err);
      setMessages(prev => [...prev, { role: 'ai', text: "Server busy hai ya limit hit ho gayi hai." }]);
    } finally {
      setIsTyping(false);
    }
  };

  return (
    <div className="flex flex-col lg:flex-row h-screen bg-[#010302] p-4 gap-6 text-zinc-200 font-sans overflow-hidden">
      
      {/* Sidebar (Chat) */}
      <div className="w-full lg:w-96 flex flex-col bg-[#0b0f0e] rounded-[2rem] border border-emerald-500/10 shadow-2xl">
        <div className="p-6 border-b border-emerald-500/5 flex items-center gap-3">
          <div className="p-2 bg-emerald-500/10 rounded-lg"><BrainCircuit className="text-emerald-400" size={20} /></div>
          <h1 className="text-xs font-black uppercase tracking-widest text-emerald-400 font-mono">Beer AI v9.0</h1>
        </div>

        <div className="flex-1 overflow-y-auto p-4 space-y-4 text-xs scrollbar-hide">
          {messages.map((msg, i) => (
            <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`p-4 rounded-2xl max-w-[85%] leading-relaxed ${
                msg.role === 'user' ? 'bg-emerald-600 text-white shadow-lg' : 'bg-[#141a18] border border-emerald-500/5'
              }`}>
                {msg.text}
              </div>
            </div>
          ))}
          {isTyping && <Loader2 className="animate-spin text-emerald-500 mx-auto" size={18} />}
          <div ref={chatEndRef} />
        </div>

        <div className="p-4 bg-black/20 space-y-3">
          <div className="flex gap-2 bg-[#020403] p-2 rounded-xl border border-emerald-500/20 focus-within:border-emerald-500 transition-all">
            <input 
              className="flex-1 bg-transparent border-none outline-none px-2 text-xs"
              placeholder="Kabeer 1000 debit..."
              value={input}
              onChange={(e) => setInput(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && handleSend()}
            />
            <button onClick={handleSend} className="bg-emerald-500 p-2 rounded-lg text-black hover:scale-95 active:scale-90 transition-all">
              <Send size={14} />
            </button>
          </div>
          <label className="flex items-center justify-center gap-2 cursor-pointer bg-white/5 p-2 rounded-xl text-[10px] font-bold uppercase hover:bg-emerald-500/10 border border-emerald-500/10 transition-all">
            <FileUp size={14} /> Upload Excel
            <input type="file" className="hidden" onChange={handleFileUpload} />
          </label>
        </div>
      </div>

      {/* Main Ledger Table */}
      <div className="flex-1 flex flex-col bg-[#0b0f0e] rounded-[2.5rem] border border-emerald-500/10 shadow-2xl overflow-hidden">
        <div className="p-6 border-b border-emerald-500/5 flex justify-between items-center bg-emerald-500/[0.02]">
          <div className="flex items-center gap-4">
            <TrendingUp className="text-emerald-500" size={24} />
            <h2 className="text-sm font-black uppercase tracking-widest italic">Financial Matrix</h2>
          </div>
          <button onClick={() => {
            const ws = XLSX.utils.json_to_sheet(processedData);
            const wb = XLSX.utils.book_new();
            XLSX.utils.book_append_sheet(wb, ws, "Audit");
            XLSX.writeFile(wb, "Beer_Ledger.xlsx");
          }} className="bg-emerald-500 text-black px-6 py-2 rounded-xl font-black text-[10px] uppercase shadow-lg hover:bg-emerald-400 transition-colors">
            Export XLSX
          </button>
        </div>

        <div className="flex-1 overflow-auto p-6 scrollbar-thin scrollbar-thumb-emerald-500/10">
          <table className="w-full border-collapse">
            <thead>
              <tr className="bg-[#121816] text-emerald-400 font-black uppercase text-[10px] border-b border-emerald-500/10 text-left">
                <th className="p-4">Date</th>
                <th className="p-4">Customer</th>
                <th className="p-4">Description</th>
                <th className="p-4 text-right">Debit (-)</th>
                <th className="p-4 text-right">Credit (+)</th>
                <th className="p-4 text-right bg-emerald-500/5 text-white font-mono">Balance</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-emerald-500/5">
              {processedData.map((row, i) => (
                <tr key={i} className="hover:bg-emerald-500/[0.02] text-[11px] transition-all group">
                  <td className="p-4 opacity-50">{row.Date || '-'}</td>
                  <td className="p-4 font-bold text-white uppercase">{row["Customer Name"] || '-'}</td>
                  <td className="p-4 opacity-70 italic">{row.Description || '-'}</td>
                  <td className="p-4 text-right text-red-400">-{row.Debit || 0}</td>
                  <td className="p-4 text-right text-emerald-400">+{row.Credit || 0}</td>
                  <td className="p-4 text-right font-black text-white bg-emerald-500/[0.03] font-mono">
                    {Number(row.Balance).toLocaleString()}
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
          {processedData.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center opacity-20 py-20">
              <TrendingUp size={60} className="mb-4" />
              <p className="text-[10px] font-black uppercase tracking-[0.5em]">No Data in Matrix</p>
            </div>
          )}
        </div>
      </div>
    </div>
  );
};

export default AiExcelManager;