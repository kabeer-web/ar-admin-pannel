...
import { Upload, Smartphone, Box, Download, Activity, Cpu, Database, Zap, Sun, Palette, RotateCcw, Share2 } from 'lucide-react';
...
const ArGenerator = () => {
  ...
  const saveMatrixConfig = async () => {
    if (!publicUrl) return alert("Pehle model injection karein!");
    setSyncing(true);
    try {
      const response = await axios.post(`${API_BASE_URL}/save-config`, {
        ...
        baseColor: baseColor,
        exposure: exposure,
        ownerId: "admin_1" // Later replace with Clerk ID
      });

      if (response.data.success) {
        setSavedId(response.data.modelId);
        alert("Matrix Synced! Database Updated ✅");
      }
    } catch (err) {
  ...
  const getARViewLink = () => {
    if (!savedId) return '';
    return `${window.location.origin}/view?id=${savedId}`;
  };
  ...
  useEffect(() => {
    ...
    if (viewer) {
      const loadHandler = () => {
        setModelStats(prev => ({ ...prev, materials: viewer.model?.materials.length || 0 }));
        if (viewer.model?.materials[0]) {
              ...
              <>
                ...
                <div className="absolute bottom-6 left-6 right-6 flex flex-wrap items-center justify-between gap-4 bg-black/60 backdrop-blur-2xl p-6 rounded-[2.5rem] border border-emerald-500/10 shadow-2xl">
                  <div className="flex items-center gap-4 bg-black/40 p-3 rounded-2xl border border-emerald-500/5">
                    ...
                    <button onClick={restoreOriginal} className="p-2 bg-emerald-500/10 hover:bg-emerald-500/20 rounded-xl text-emerald-400 transition-all flex items-center gap-2 text-[10px] font-black uppercase tracking-widest border border-emerald-500/20">
                      <RotateCcw size={14}/>
                    </button>
                  </div>
            ...
            {savedId ? (
              <div className="flex flex-col items-center gap-10 w-full z-10">
                ...
                <div className="flex flex-col gap-3 w-full">
                  <button onClick={downloadQRCode} className="w-full bg-emerald-600 hover:bg-emerald-400 py-6 rounded-3xl font-black text-black flex items-center justify-center gap-3 uppercase tracking-widest transition-all">
              ...
              <div className="opacity-10 flex flex-col items-center gap-8 py-20">
                ...
                <p className="font-black tracking-[0.4em] uppercase text-xs leading-loose">Waiting for Sync</p>
              </div>
            )}
          </div>
        </div>
